...
Секція «Коментарі» (`src/components/Comments.astro`, унизу головної сторінки) — публічна форма з обов'язковими полями «Ім'я» та «Email». Коментар публікується одразу, без модерації; **email ніколи не показується на сайті** — зберігається лише на бекенді, щоб можна було зв'язатися або відрізнити спам.
...
- `POST /comments` — новий коментар `{ name, email, text }`.
